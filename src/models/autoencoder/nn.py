...
class Stretch(nn.Module):
    ...
    def __init__(self, num_features, num_dims, alpha):
        ...
        if alpha is None:
            self.alpha = (0.99 ** th.arange(shape[1])).view(shape)
        else:
            self.alpha = alpha
        self.gamma = nn.Parameter(0.01*th.ones(shape))
        self.beta = nn.Parameter(th.pi*th.ones(shape))
        self.register_buffer('moving_mag', 1.*th.ones(shape))
        self.register_buffer('moving_min', th.pi*th.ones(shape))
...
def checkpoint(func, inputs, params, flag):
    ...
    """
    if flag and any(param.requires_grad for param in params):
...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        ...
        # This when time embedding is off.
        if emb is not None:
...
class MiddleBlock(nn.Module):
    def __init__(self, ch, time_embed_dim, dropout, dims, use_checkpoint, use_scale_shift_norm, 
                 use_time_embed=False, make_vae=False, num_heads=None, num_head_channels=None, 
                 use_new_attention_order=None):
        super(MiddleBlock, self).__init__()
        
        # Select the appropriate sequential block type
        SequentialBlock = TimestepEmbedSequential if use_time_embed else nn.Sequential

        # Define the core middle block sequence
        self.middle_block = SequentialBlock(
            ResBlock(
        ...
        if self.make_vae:
            self.flatten = nn.Flatten()  # Flatten the output before feeding it to mu and sigma layers
            # Adjust the input dimension here based on the flattened size
            flattened_dim = 4096  # hardcoded for now
            self.mu_layer = nn.Linear(flattened_dim, flattened_dim)
            self.logvar_layer = nn.Linear(flattened_dim, flattened_dim)

    def forward(self, x):
        # Pass through the middle block
        x = self.middle_block(x)

        # If VAE, produce mu and sigma as separate outputs
        if self.make_vae:
            x = self.flatten(x)
            mu = self.mu_layer(x)
