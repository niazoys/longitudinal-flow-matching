...
def parse_input(x_in_: torch.Tensor, dim: int,_reshape):
    ...
    time_kp1 = x_in_[:, -3]
    label    = x_in_[:, -7:-3]
    x_in     = torch.cat([x_in_[:, :-7], x_in_[:, -1].unsqueeze(1)], dim=1)
...
class VectorFieldRegressorBottleNeckCond(nn.Module):
    def __init__(
            self,
            depth: int,
            mid_depth: int,
        ...
        super(VectorFieldRegressorBottleNeckCond, self).__init__()
        print("VectorFieldRegressorBottleNeckCond")
        ...
        # Add new projection layers for secondary conditioning
    ...
    def forward(self, x_in_: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Output tensor with format [B, ...]
        """
        # Map inputs to VectorFieldRegressor format
        input_latents, reference_latents, conditioning_latents, index_distances, timestamps,t_future,label = \
        ...
        out = self._forward(
            ...
            label=label,
            future_time=t_future)

        return torch.cat([out.reshape(x_in_.size(0), -1), t_future.view(-1,1)], dim=-1)
    ...
    def _forward(
            ...
            label: torch.Tensor,
            future_time: torch.Tensor) -> torch.Tensor:
        """
        ...
        :param timestamps: [b]
        :param label: [b, 4]
        :param future_time: [b] - The time delta to predict into the future
        :return: [b, c, h, w]
        """
        ...
        dist = self.time_projection(torch.sqrt(index_distances).unsqueeze(1)).unsqueeze(1)
        future_mask = (future_time > 1e-5).float().unsqueeze(1)
        safe_future_time = torch.clamp(future_time, min=1e-5)
        raw_future_embedding = self.time_projection(torch.sqrt(safe_future_time).unsqueeze(1))  # [28, 512]
        future_dist = raw_future_embedding.unsqueeze(1) * future_mask.unsqueeze(-1)  # [28, 1, 512]
        ...
        for block in self.in_blocks:
            ...
            hs.append(x.clone())
        x = self.mid_blocks(x)
        # Apply secondary conditioning after mid blocks
        # Transform conditioning through the secondary projection layers
        secondary_dist = self.secondary_time_projection(dist.squeeze(1)).unsqueeze(1)
        secondary_future_dist = self.secondary_time_projection(raw_future_embedding).unsqueeze(1) * future_mask.unsqueeze(-1)
        ...
        x[:, 1:] = x[:, 1:] + secondary_dist + secondary_cls + secondary_future_dist
    ...
    def map_to_vector_field_regressor(self, x_in_):
        ...
        time_future = parsed["time_kp1"]-timestamps
        return input_latents, reference_latents, conditioning_latents, index_distances, timestamps, time_future,label
