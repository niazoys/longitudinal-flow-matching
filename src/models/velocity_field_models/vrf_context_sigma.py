...
class VectorFieldRegressorSigma(nn.Module):
    def __init__(
            self,
            depth: int,
            mid_depth: int,
        ...
        super(VectorFieldRegressorSigma, self).__init__()
        print("VectorFieldRegressor w/ Demographic Conditioning and Sigma Prediction")
    ...
    def forward(self, x_in_: torch.Tensor) -> torch.Tensor:
        ...
        # Concatenate mean and sigma along the channel dimension
        outputs = torch.cat([mean, sigma], dim=1)
    ...
    def _forward(
        ...
        # Project to output - both mean and sigma
        mean = self.project_out(x[:, 1:])
        sigma = self.project_sigma(x[:, 1:])
