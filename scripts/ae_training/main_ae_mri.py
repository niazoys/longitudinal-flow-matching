...
from utils.prepare_dataset import prepare_dataset_AE


torch.set_float32_matmul_precision('medium')
DEFAULT_CONFIG = os.path.join(ROOT, "configs/ae_gbm.yaml")


def flatten_ae_config(config: ml_collections.ConfigDict) -> ml_collections.ConfigDict:
    flat = ml_collections.ConfigDict()
    flat.dataset_name = config.dataset.name
    flat.dataset_path = config.dataset.dataset_path.replace('/ivi/zfs/s0/original_homes/mislam', ROOT)
    flat.image_folder = config.dataset.image_folder
    flat.image_size = tuple(config.model.image_size)
    flat.target_dim = tuple(config.model.image_size)
    flat.max_slice_per_patient = config.dataset.max_slice_per_patient
    flat.use_only_pathological_case = config.dataset.use_only_pathological_case
    flat.train_val_test_ratio = config.data.train_val_test_ratio
    flat.max_training_samples = config.data.max_training_samples
    flat.normalize_zero_to_one = config.data.normalize_zero_to_one
    flat.load_in_triplet = config.data.get('load_in_triplet', 'triplet')
    flat.batch_size = config.training.batch_size
    flat.num_workers = config.training.num_workers
    return flat
...
def main(config: ml_collections.ConfigDict):
    ...
    train_transform = A.Compose(
        [
            ...
            A.ShiftScaleRotate(shift_limit=0.15, scale_limit=0.1, rotate_limit=30, p=0.25),
    ...
    # Load the data
    dataset_config = flatten_ae_config(config)
    train_loader, val_loader, test_loader, num_image_channel, max_t, train_pid, val_pid, test_pid = \
        prepare_dataset_AE(config=dataset_config, transforms_list=transforms_list)
    # Hardware settings
...
if __name__ == "__main__":
    ...
    print_config(config, title="GBM Autoencoder")
    main(config)
